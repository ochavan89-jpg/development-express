      ...
      {!collapsed && (
        <div style={{ position:'absolute', bottom:14, left:0, right:0, padding:'0 12px' }}>
          <div style={{ background:'rgba(201,168,76,0.05)', border:'1px solid var(--border)', borderRadius:8, padding:'9px 12px' }}>
            ...
            <div style={{ fontSize:12, color:'var(--gold)', fontWeight:700 }}>{user?.full_name || 'Unknown user'}</div>
            <div style={{ fontSize:10, color:'var(--text-dim)', textTransform:'uppercase', letterSpacing:1 }}>{user?.role || 'unknown'}</div>
          </div>
        </div>
      )}
    </aside>
...
function Header({ onToggle }) {
  ...
  const initials = (user?.full_name || 'User').split(' ').map(n=>n[0]).join('').slice(0,2)
      ...
      <div style={{ display:'flex', alignItems:'center', gap:12 }}>
        <div style={{ background:'rgba(201,168,76,0.05)', border:'1px solid var(--border)', borderRadius:8, padding:'6px 12px', display:'flex', alignItems:'center', gap:10 }}>
          <div style={{ textAlign:'right' }}>
            <div style={{ fontSize:13, fontWeight:600, color:'var(--gold)' }}>{user?.full_name || 'Unknown user'}</div>
            <div style={{ fontSize:10, color:'var(--text-dim)' }}>
              {user?.role === 'admin' ? 'B.Tech Civil Engg. · MD' : user?.role || 'unknown'}
            </div>
