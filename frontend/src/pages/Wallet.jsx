...
export default function Wallet() {
  ...
  useEffect(() => {
    ...
    const balances = user.role === 'admin'
      ? walletAPI.getAllBalances().then(r=>setBals(r.data.data || []))
      : walletAPI.getBalance().then(r=>setBals([{
          id: user.id,
          full_name: user.full_name,
          company_name: user.company_name || user.full_name,
          wallet_balance: r.data.data?.balance || 0,
        }]))
    balances.catch(()=>setBals(user.role === 'admin' ? DEMO_BAL : []))
    walletAPI.getTransactions({limit:20}).then(r=>setTxns(r.data.data || [])).catch(()=>setTxns(user.role === 'admin' ? DEMO_TXN : []))
  }, [user])
