...
const getJwtSecret = () => {
  const secret = process.env.JWT_SECRET || (!isProduction() ? 'devexpress_fallback_secret' : null)
  if (!secret) {
...
const protect = async (req, res, next) => {
  try {
    ...
    try {
      ...
      // Local demo fallback only. Production must not trust role claims from the token alone.
