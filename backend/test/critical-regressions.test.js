...
test('does not debit wallet when completing a cancelled booking', async () => {
  process.env.NODE_ENV = 'test'
  const queries = []
  setDbQuery(async () => ({
    rows: [{
  ...
  const response = await request('/api/bookings/124/complete', {
    method: 'PUT',
    token,
    body: { actual_hours: 5 },
  })
  ...
  assert.equal(response.data.message, 'Only active bookings can be completed')
  assert.equal(queries.some(sql => /UPDATE de_users/.test(sql)), false)
  assert.equal(queries.some(sql => /INSERT INTO wallet_transactions/.test(sql)), false)
})
