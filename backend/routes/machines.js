...
router.put('/:id', protect, authorize('admin','owner'), async (req, res) => {
  try {
    ...
    if (req.user.role === 'owner') {
      const { rows: owned } = await pool.query('SELECT id FROM machines WHERE id=$1 AND owner_id=$2', [req.params.id, req.user.id])
      if (!owned.length) return res.status(403).json({ success:false, message:'Cannot update another owner machine' })
    }
