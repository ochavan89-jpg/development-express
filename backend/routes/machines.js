...
router.put('/:id', protect, authorize('admin','owner'), async (req, res) => {
  try {
    ...
    if (req.user.role === 'owner') {
      ...
      ownerPredicate = ` AND owner_id=$${vals.length}`
    }
    const { rows } = await pool.query(`UPDATE machines SET ${updates.join(',')} WHERE id=$${idParamIndex}${ownerPredicate} RETURNING *`, vals)
    if (!rows.length) return res.status(404).json({ success:false, message:'Machine not found' })
