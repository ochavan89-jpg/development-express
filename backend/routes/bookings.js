...
router.post('/', protect, authorize('admin','client'), async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      ...
      [client_id, machine_id, operator_id, start_time, estimated_hours, machines[0].rate_per_hour, site_address, work_description]
    )
...
router.put('/:id/complete', protect, authorize('admin','operator'), async (req, res) => {
  ...
  try {
    ...
    if (!isPositiveNumber(actual_hours)) return res.status(400).json({ success:false, message:'Actual hours must be greater than zero' })
    ...
    const totalAmount = Number(actual_hours) * Number(booking.hourly_rate)
    const { rows: users } = await client.query('SELECT wallet_balance FROM de_users WHERE id=$1 FOR UPDATE', [booking.client_id])
    if (!users.length) {
      await client.query('ROLLBACK')
    ...
    await client.query(
      ...
      [booking.client_id, totalAmount, balanceBefore, balanceAfter, req.params.id]
    )
...
router.put('/:id/cancel', protect, async (req, res) => {
  try {
    ...
    const booking = br[0]
    const canCancel = req.user.role === 'admin' || Number(booking.client_id) === Number(req.user.id) || Number(booking.owner_id) === Number(req.user.id)
    if (!canCancel) return res.status(403).json({ success:false, message:'Cannot cancel this booking' })
    const { rowCount } = await pool.query("UPDATE bookings SET status='cancelled' WHERE id=$1 AND status <> 'completed'", [req.params.id])
    if (!rowCount) return res.status(400).json({ success:false, message:'Completed booking cannot be cancelled' })
