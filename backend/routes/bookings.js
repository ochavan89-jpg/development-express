...
router.put('/:id/complete', protect, authorize('admin','operator'), async (req, res) => {
  ...
  try {
    ...
    if (br[0].status === 'completed') {
      ...
      return res.json({ success:true, data: br[0] })
    }
    ...
    await client.query(
      `INSERT INTO wallet_transactions (user_id,transaction_type,amount,balance_before,balance_after,description,reference_id)
       SELECT id,'debit',$1,wallet_balance + $1,wallet_balance,'Booking completion',$2
       FROM de_users WHERE id=$3`,
      [totalAmount, req.params.id, br[0].client_id]
    )
    await client.query('COMMIT')
...
router.put('/:id/cancel', protect, async (req, res) => {
  try {
    ...
    const allowed =
      req.user.role === 'admin' ||
      (req.user.role === 'client' && Number(br[0].client_id) === Number(req.user.id)) ||
      (req.user.role === 'owner' && Number(br[0].owner_id) === Number(req.user.id))
    if (!allowed) return res.status(403).json({ success:false, message:'Access denied' })
    const { rowCount } = await pool.query(
      "UPDATE bookings SET status='cancelled' WHERE id=$1 AND status <> 'completed'",
      [req.params.id]
    )
    if (!rowCount) return res.status(400).json({ success:false, message:'Completed bookings cannot be cancelled' })
