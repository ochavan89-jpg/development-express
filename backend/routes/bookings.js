  ...
  try {
    ...
    await client.query(
      `INSERT INTO wallet_transactions (user_id,transaction_type,amount,balance_before,balance_after,description,booking_id)
       SELECT id,'debit',$1,wallet_balance+$1,wallet_balance,'Booking completion charge',$2 FROM de_users WHERE id=$3`,
...
router.put('/:id/cancel', protect, async (req, res) => {
  try {
    const { rows: br } = await pool.query(
      `SELECT b.client_id, m.owner_id machine_owner_id
       FROM bookings b
    ...
    const allowed =
      ...
      (req.user.role === 'owner' && booking.machine_owner_id === req.user.id)
