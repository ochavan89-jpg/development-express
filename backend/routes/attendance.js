...
router.put('/:id/punch-out', protect, authorize('operator'), async (req, res) => {
  try {
    ...
    if (ar[0].punch_out_time) return res.status(409).json({ success:false, message:'Already punched out' })
    const hrs = (new Date() - new Date(ar[0].punch_in_time)) / 3600000
    const { rows } = await pool.query(
      'UPDATE attendance SET punch_out_time=NOW(), total_hours=$1 WHERE id=$2 AND operator_id=$3 AND punch_out_time IS NULL RETURNING *',
      [hrs.toFixed(2), req.params.id, req.user.id]
    )
    if (!rows.length) return res.status(409).json({ success:false, message:'Already punched out' })
