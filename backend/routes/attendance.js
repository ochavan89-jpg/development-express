...
router.put('/:id/punch-out', protect, authorize('operator'), async (req, res) => {
  try {
    ...
    if (Number(ar[0].operator_id) !== Number(req.user.id)) {
      return res.status(403).json({ success:false, message:'Cannot punch out another operator record' })
    }
    if (ar[0].punch_out_time) {
      return res.status(409).json({ success:false, message:'Attendance record already punched out' })
    }
