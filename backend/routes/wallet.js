...
router.post('/recharge', protect, authorize('admin'), async (req, res) => {
  ...
  try {
    ...
    if (!user_id || !Number.isFinite(rechargeAmount) || rechargeAmount <= 0) {
      return res.status(400).json({ success:false, message:'Invalid recharge data' })
    }
    ...
    const { rows: ur } = await client.query(
      ...
      [rechargeAmount, user_id]
    )
    if (!ur.length) {
      await client.query('ROLLBACK')
      return res.status(404).json({ success:false, message:'User not found' })
    }
    const balBefore = ur[0].balance_before
    const balAfter  = ur[0].balance_after
