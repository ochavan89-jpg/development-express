...
router.post('/recharge', protect, authorize('admin'), async (req, res) => {
  ...
  try {
    ...
    if (!user_id || !Number.isFinite(rechargeAmount) || rechargeAmount <= 0) return res.status(400).json({ success:false, message:'Invalid recharge data' })

    client = await pool.connect()
    await client.query('BEGIN')
    const { rows: ur } = await client.query("SELECT wallet_balance FROM de_users WHERE id=$1 AND role='client' FOR UPDATE", [user_id])
    if (!ur.length) {
